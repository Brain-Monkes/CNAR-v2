...
import { MapPin, Navigation, Loader2, X, Crosshair, Radio } from 'lucide-react';
...
export function RoutePlanner() {
  const {
    ...
    showTowers, setShowTowers, fetchHeatmap, heatmapData,
  } = useRouting();

  const [originInput, setOriginInput] = useState('');
  const [destInput, setDestInput] = useState('');

  // Load heatmap data for tower overlay when toggled on
  useEffect(() => {
    if (showTowers && heatmapData.length === 0) {
      fetchHeatmap();
    }
  }, [showTowers, heatmapData.length, fetchHeatmap]);
  ...
  const handleGeocode = async (type: 'origin' | 'destination') => {
    ...
    try {
      ...
      if (result) {
        if (type === 'origin') {
          setOrigin([result.lat, result.lon], result.label);
          setOriginInput(result.label);
        } else {
          setDestination([result.lat, result.lon], result.label);
          setDestInput(result.label);
        }
      }
    } catch { /* silently fail */ }
  };
      ...
      <form onSubmit={handleSubmit} className="route-planner-form">
        ...
        <div className="input-group">
          <label className="input-label">
            <MapPin size={14} className="label-icon origin-icon" />
            Origin
          </label>
          <div className="input-row">
            <input
              ...
              placeholder={originLabel || 'Search location...'}
              value={originInput}
              onChange={(e) => setOriginInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleGeocode('origin'))}
            />
            <button
              type="button"
              onClick={() => setSelectionMode(selectionMode === 'origin' ? null : 'origin')}
              className={`pin-btn ${selectionMode === 'origin' ? 'active' : ''}`}
              title="Pin on Map"
            >
              <Crosshair size={16} />
            </button>
        ...
        <div className="input-group">
          <label className="input-label">
            <Navigation size={14} className="label-icon dest-icon" />
            Destination
          </label>
          <div className="input-row">
            <input
              type="text"
              className="text-input"
              placeholder={destinationLabel || 'Search location...'}
              value={destInput}
              onChange={(e) => setDestInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleGeocode('destination'))}
            />
            <button
              type="button"
              onClick={() => setSelectionMode(selectionMode === 'destination' ? null : 'destination')}
              className={`pin-btn ${selectionMode === 'destination' ? 'active' : ''}`}
              title="Pin on Map"
            >
              <Crosshair size={16} />
            </button>
        ...
        <label className="tower-toggle">
          ...
          />
          <Radio size={14} />
          <span>Show Tower Clusters on Map</span>
        </label>
          ...
          <div className="route-summary">
            ...
            <div className="summary-chips">
              <div className="chip">
                ...
                <span className="chip-value">{topRoute.distance_km} km</span>
              </div>
              <div className="chip">
                <span className="chip-label">Duration</span>
                <span className="chip-value">{topRoute.duration_min} min</span>
              </div>
              <div className="chip">
                <span className="chip-label">Signal</span>
                <span className="chip-value" style={{ color: getConnectivityColor(topRoute.connectivity_score) }}>
                  {getConnectivityLabel(topRoute.connectivity_score)}
                </span>
              </div>
            </div>
          </div>
